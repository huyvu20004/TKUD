...
function checkResize() {
    ...
    map_all_btn();
    nav_to_left();
    handle_page_nav_circle($('.page-nav__no.this-active').attr('data-id'));
    nav_home();
    nav_responsive();
    nav_blog();
    nav_mouse();
...
$(function() {
    ...
    if ($('.nav-wrapper').hasClass('this-active')) {} else if ($('#rotating-intro').length == 0) {
        clearLogoInterval();
        setupLogoInterval();
    }
...
function handle_page_nav_no() {
    ...
    $('.page-nav__no').mouseenter(function() {
        if ($('body').hasClass('hashover')) {
            ...
            $('.page-nav__lbl[data-id="' + circleMoveIndex + '"]').css({
                opacity: '1'
            });
        }
    });
...
function handle_eqmt_img() {
    if ($('.eqmt-img').length > 0 && ww >= 992) {
        $(window).on('scroll', function() {
            setTimeout(function() {
                if ($('.eqmt-list').parent('.js-section').hasClass('this-active')) {} else {
...
function nav_logo() {
    ...
    $('.nav-logo').click(function() {
        if (ww < 992) {
            ...
            $(this).addClass('this-active');
            clearLogoInterval();
            topD = $('.pos-marker').offset().top - $(this).offset().top;
            $(this).css({
                'transform': 'translateX(-50%) translateY(' + topD + 'px)'
            });
        }
    });
    $(window).on('scroll', function() {
    ...
    $('.nav .nav__item').mouseenter(function() {
        if (ww >= 992 && $('body').hasClass('hashover') && $('.nav-wrapper').hasClass('center')) {
            ...
            $('.nav-hover__text-item[data-id="' + hoverIndex + '"]').css({
                top: thisPos
            });
        }
        $('.nav .nav__item').css({
    ...
    $('.nav .nav__item').mouseleave(function() {
...
function nav_mouse() {
    ...
    if (ww >= 992) {
        $('.nav-wrapper').mouseenter(function() {
            if ($(this).hasClass('this-active')) {} else {
                $('#mouse').addClass('nav');
            }
        });
        ...
        $('.nav-wrapper__logo, .nav-cart-btn').mouseleave(function() {
            if ($('.nav-wrapper').hasClass('this-active')) {} else {
                $('#mouse').addClass('nav');
            }
        });
    }
